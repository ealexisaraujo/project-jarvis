...
and a real, tappable Wi-Fi station connection. After Wi-Fi comes online, a
non-blocking SNTP service obtains America/Phoenix time, writes it to the RTC,
and accepts the result only after validated readback. Deterministic tile
selection supports framebuffer QA for every screen. A battery and microSD card
are not required.
...
fixed Arizona timezone and initializes the PCF85063 once per powered boot; the
Clock continues to read the RTC once per second. Weather data and the four
application tiles remain placeholders. The firmware also reports the detected
chip, flash, PSRAM, heap, display, touch, RTC, Wi-Fi, and time-sync state plus
an uptime heartbeat over USB serial. Battery, audio, microphone, IMU, and
microSD support remain outside this milestone.
...
time_sync_status=waiting_wifi|requested timezone=America_Phoenix|online
...
## Milestone 4.1 UI, Wi-Fi, and time state
...
- Weather shows `Ashford`, station connection progress/status, and an optional
  local IPv4 address. Temperature and metrics remain `--`; no weather API is
  called.
...
the Wi-Fi snapshot to become online, calls `configTzTime()` with POSIX timezone
`MST7` and two public NTP servers, and checks `time()`/`localtime_r()` without a
blocking `getLocalTime()` timeout. RTC initialization proceeds only after SNTP
reports a completed synchronization. `MST7` represents America/Phoenix at
UTC-7 with no daylight-saving transition. Years before 2024 are rejected. A
Wi-Fi disconnect before completion returns the service to a retryable waiting
state; after one verified RTC update it performs no further synchronization
that boot.
...
The hardware-free validation commands for this milestone are:
...
5. NTP-backed America/Phoenix RTC initialization (software implemented and
   hardware-free validation complete; device proof pending)
6. Live weather plus Bins, alarm, radio, and MP3 application services
7. Optional audio, microphone, IMU, microSD storage, and battery/power
   management
